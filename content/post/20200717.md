...
ずーーーーっとハマっていたことがついに、遂に!!!できたので忘れないように書いております。別にサボっていたわけではないのです(*'ω'*)

いや、今回はなかなかハードモードでして。。。(とってもツカレタ)
...
知らない人のために書くとこれがラズパイってやつです。今回は彼に頑張っていただきVPNを張ります。笑
...
私は基本的にはテザリングとかでつなげてます。ですがふとした時につなげることも合ったりなかったり。ただ繋げる想定がある以上、言うまでもなくセキュリティー的には良くないんですよね。特にフリーWifiとかは暗号化されていないので成りすましやのぞき見などのリスクが高まるってワケです。
...
### OSのインストール
Raspbbery piにOSをダウンロードしなければいけませんが、別用途で使うために先にosインストールしてました。実際のところふつうにRaspbian（ラズビアン）で正常に動くので問題ないかと思います。

情報量的にはSoftetherを使ってやっているかたも多いみたいですがある事情で今回は見送りました。それについては後で説明しようかなぁあと

### インストール
[https://github.com/adrianmihalko/raspberrypiwireguard](https://github.com/adrianmihalko/raspberrypiwireguard)

基本的にはこの資料を見れば結構できちゃうと思います。
(だた英語なのでちょっと抵抗ある方はGoogle翻訳していただければと思います。

個人的には[こちら](https://blog.koh.dev/2020-01-01-vpn/)の方の資料もわかりやすかったので一応共有しておきます。

いや~ほんとに先人の方たちの知恵に感謝ですね。

とりあえず以下のコマンドをそのまま流していけると思います。自分は権限の都合上sudoつけてますが個人の環境によって異なると思いますので読み替えて実施していただければよいです。
...
`sudo apt-get update`と`sudo apt-get upgrade`が結構時間かかりました。結構放置してたからかも。
コーヒーでも入れて終わるまで少し待ちましょう

続いて再起動後、トイレに行ってを進めます。
...
再起動後には設定の確認が必要かと思いますので下記のコマンドで確認します。
```
...
こういうミスの場合頭をつかっておらずコピペしたところとかが結構怪しかったりするもんです。そうです、自分には心当たりがありました。
VPNServerのインターフェース周りの設定を見てみます。
...
この`eth0`は実際のNICに書き換えてやらないといけないんですが書き換える場合ってRaspbbery piを無線でつなげてる時ぐらいだろ。。とか思ってたわけですよ。いあ???まてよ。

以前SoftEtherの導入の際に仮想のNICを作ったような消したようなというぬるい記憶が蘇って来たので確認してみます。
